...
class Application extends SQLData{
	...
	function isUserOnline(){
		global $LOCALE;
		if($this->session->get($this->config['SESSION_NAME']."_dashboard")) return true;
		return false;
		
	}
	
	function getUserOnline(){
		if($this->session->get($this->config['SESSION_NAME']."_dashboard")) return json_decode(urldecode64($this->session->get($this->config['SESSION_NAME']."_dashboard")));
		else return false;
