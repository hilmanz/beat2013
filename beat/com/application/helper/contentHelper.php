...
class contentHelper {
	...
	function getAuthorProfile($otherid=null,$typeAuthor='admin'){
		...
		foreach($data as $key => $val){
			...
			if($val['img']) $data[$key]['image_full_path'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/user/photo/".$val['img'];
	...
	function getArticleContent($start=null,$limit=10,$contenttype=0,$topcontent=array(0,3),$articletype=false,$groupby=false,$author=false) {
		global $CONFIG;
		...
		if ($this->apps->_p('startdate')) {
			...
			$startdate = "AND anc.posted_date >= '{$start_date}' ";
		}
		if ($this->apps->_p('enddate')) {
			$end_date = $this->apps->_p('enddate');
			$enddate = "AND anc.posted_date < '{$end_date}' ";
		}
		$authorid = "AND anc.authorid = {$this->uid}";
		...
		if($filter['uidsearch']!='') $qHirarki = " {$filter['uidsearch']} ";
			...
			if($leadertype){
						...
						if(in_array($articletype,$this->modzeropage)){
							...
							$auhtorminion = @$this->apps->user->masterdetail;
							if($auhtorminion){
								foreach($auhtorminion as $val){
										$auhtorarrid[$val->ownerid] = $val->ownerid;
								}
		...
		if($articletype=='plan'){
			$qStatus = "  AND n_status IN (0,1,2) ";		
		}
		
		//GET TOTAL ARTICLE

		$sql = "SELECT count(*) total FROM {$this->dbshema}_news_content anc  WHERE articleType IN ({$typeid}) AND topcontent IN ({$topcontent}) {$search} {$startdate} {$enddate} {$filter['keywordsearch']} {$filter['categorysearch']} {$filter['citysearch']} {$filter['postedsearch']} {$filter['fromwhosearch']} {$qStatus} {$qHirarki} 
		...
		$sql = "
			SELECT id,title,brief,image,thumbnail_image,slider_image,posted_date,expired_date,file,url,fromwho,tags,authorid,topcontent,cityid ,articleType,can_save,n_status
			FROM {$this->dbshema}_news_content anc
			{$filter['subqueryfilter']} 
			WHERE articleType IN ({$typeid}) AND topcontent IN ({$topcontent}) {$search} {$startdate} {$enddate} {$filter['keywordsearch']} {$filter['categorysearch']} {$filter['citysearch']} {$filter['postedsearch']} {$filter['fromwhosearch']} {$qStatus} {$qHirarki} 
			{$filter['groupbyfilter']} ORDER BY {$filter['suborderfilter']}  posted_date DESC , id DESC 
			LIMIT {$start},{$limit}";
		// pr($sql);
		$rqData = $this->apps->fetch($sql,1);
	...
	function getCommentModeration($start=null,$limit=10,$type=null) {
		...
		// GET CONTENT POST
		$sql_content = "SELECT id,title FROM {$this->dbshema}_news_content WHERE authorid = {$userid} AND articleType = {$type} AND n_status = 1";
	...
	function getStatistictArticle($rqData=null){
		...
		foreach($qData as $key => $val){
				//user profile
				 if(array_key_exists($val['authorid'],$socialProfile)) $qData[$key]['author'] = $socialProfile[$val['authorid']];		
				//city data
		...
		if($commentsData){
			$commentsDataComment = $this->getComment($cidStr,false,0,2);
			foreach($qData as $key => $val){
				if ($commentsData) {
		...
		if($gallerydata){
			...
			foreach($qData as $key => $val){
				...
				if(array_key_exists($val['id'],$gallerydata)) $qData[$key]['totalgallery'] = count($gallerydata[$val['id']])+1;
				else $qData[$key]['totalgallery'] = 1;
		...
		//get friend tags
		$friendtagsData = $this->getFriendTagged($cidStr);
		if($friendtagsData){
	...
	function addUploadImage($data=false,$type=NULL){
		...
		$sql ="
			INSERT INTO {$this->dbshema}_news_content (cityid,brief,title,content,tags,image,articleType,created_date,posted_date,expired_date,authorid,fromwho,n_status,url) 
			VALUES ('{$city_event}','{$brief}','{$title}','{$description}','{$tags}','{$image}',{$type},NOW(),'{$posted_date}','{$expired_date}','{$authorid}','{$fromwho}',{$this->moderation},\"{$url}\")
			";
					...
					if(is_array($arrfid)){
						...
						if($frienddata){
							...
							foreach($frienddata as $val){
								$this->addFriendTags($cid,$val['fid'],$val['ftype'],false);
								$this->apps->messageHelper->createMessage($fid," visit {$title} at {$brief} {$posted_date} ");	
							}
						
						}
					}else{
						$ftype = intval($ftype);
						$this->addFriendTags($cid,intval($fid),intval($ftype),false);						
						$this->apps->messageHelper->createMessage($fid," visit {$title} at {$brief} {$posted_date} ");	
					}
			}
	...
	function getFriendTagged($strCid=null,$tagin=0){
		if(!$this->uid) return false;
		...
		WHERE userid={$this->uid} AND contentid IN ({$strCid}) AND n_status = 1 AND tagin = {$tagin} ";
		...
		$arrEntourageFid = false;
				...
				if($arrEntourageFid) {
					...
					$entouragefid = $this->apps->userHelper->entouragedata($strentouragefid);
					if($entouragefid){
	...
	function addFriendTags($cid=0,$fid=0,$ftype='pass',$gallery=false){
	...
	function unFriendTags($gallery=false){
	...
	function unContentPost(){
		...
		$sql = "UPDATE {$this->dbshema}_news_content SET n_status = 0 WHERE id = '{$cid}' AND fromwho=1 AND articleType={$type} AND authorid IN ({$this->uid}) LIMIT 1";
		if ($this->apps->query($sql)) {
			return true;
		} else return false;
		return false;
	...
	function unCommentPost(){
		...
		$sql = "UPDATE {$this->dbshema}_news_content_comment SET n_status = 0 WHERE id = '{$id}' LIMIT 1";		
		if ($this->apps->query($sql)) {
			return true;
		} else return false;
		return false;
				...
				if($friendtags){					
					...
					if(is_array($arrfid)){
						...
						if($frienddata){
							...
							foreach($frienddata as $val){
								$this->addFriendTags($galleryid,$val['fid'],$val['ftype'],true);
							}
						
						}
					}else{
						$friendtypetags = intval($friendtypetags);
						$this->addFriendTags($galleryid,intval($friendtags),intval($friendtypetags),true);
					}
				}
	...
	function editContentArticle($data=false){
		...
		if ($this->apps->query($sql)) {
			...
			else $message = " visit {$qSet['title']} at {$qSet['brief']} {$qSet['posted_date']} ";
					...
					if(is_array($arrfid)){
						...
						if($frienddata){
							...
							foreach($frienddata as $val){
								...
								$this->addFriendTags($id,$val['fid'],$val['ftype'],false);
								$this->apps->messageHelper->createMessage($val['fid'],$message);
							}
						...
						$this->addFriendTags($id,intval($fid),intval($ftype),false);						
						$this->apps->messageHelper->createMessage($fid,$message);
					}
			}
	...
	function getCheckin($cidstr=false){
		...
		$sql = " SELECT * FROM my_checkin WHERE contentid in ({$cidstr})  ";
